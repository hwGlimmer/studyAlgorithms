...
//           getDistanceAndHight(scanner);
    ...
    public static Integer findOddTimes(Integer[] arr) {
        ...
        for (Integer i : arr) {
            ...
            // 一个数和0异或等于本身， 和自己异或等于0
            // 所以只要是偶数，会互相抵消变为0， 只有刚好奇数时和0异或等于自己
            count ^= i;
        }
    ...
    public static Integer divideGold(Integer[] arr) {
        ...
        for (Integer target : arr) {
            minimumHeap.offer(target);
        }

        while (minimumHeap.size() > 1) {

            usedMoney += minimumHeap.poll() + minimumHeap.poll();

            minimumHeap.offer(usedMoney);
    ...
    private static Map<Integer, TreeNode> getIntegerTreeNodeMap(int n, Scanner scanner) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (lch != 0 && !map.containsKey(lch)) {
                map.put(lch, new TreeNode(lch));
            }
            if (rch != 0 && !map.containsKey(rch)) {
                map.put(rch, new TreeNode(rch));
            }
            TreeNode node = map.get(fa);
            if (lch != 0) {
                node.left = map.get(lch);
            }
            if (rch != 0) {
                node.right = map.get(rch);
            }
        }
        return map;
    }
    ...
    private static Integer maxScoreCardGame(Integer[] arr) {
        ...
        result = Math.max(f[0][arr.length - 1], s[0][arr.length - 1]);
